...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "Metadata.steelConstruction" });
...
const imageItems = [
  {
    id: 1,
    url: "https://res.cloudinary.com/dstwpqgrz/image/upload/v1783273665/celik_kont_or9lhd.webp",
...
export default async function Page({ params }: Props) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "Pages.steelConstruction" });
  const b = await getTranslations({ locale, namespace: "Breadcrumb" });
