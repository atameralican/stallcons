...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "Metadata.craneSystems" });
...
const imageItems = [
  {
    id: 1,
    url: "https://res.cloudinary.com/dstwpqgrz/image/upload/v1783273665/maden_or4qrd.webp",
...
export default async function Page({ params }: Props) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "Pages.craneSystems" });
  const b = await getTranslations({ locale, namespace: "Breadcrumb" });
