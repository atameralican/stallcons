...
export function ProjectAdminClient({ initialProjects }: { initialProjects: ProjectRecord[] }) {
    ...
    const [isFormOpen, setIsFormOpen] = useState(initialProjects.length === 0);
    ...
    async function refreshProjects() {
        ...
        if (!response.ok) {
            setMessage({ type: "error", text: result.error ?? "Projeler alınamadı." });
            return;
        }
    ...
    function startEdit(project: ProjectRecord) {
        ...
        setForm({
            ...
            weight_tons: project.weight_tons?.toString() ?? "",
    ...
    const handleCoverChange: UploadProps<UploadResponse>["onChange"] = (info) => {
        const { status, name, response } = info.file;
        ...
        if (status === "done" && response?.url) {
            ...
            messageApi.success(`${name} kapak görseli olarak yüklendi.`);
        } else if (status === "error") {
            messageApi.error(response?.error ?? `${name} yüklenemedi.`);
        }
    };
    ...
    async function handleSubmit(event: FormEvent<HTMLFormElement>) {
        ...
        if (!slug) {
            setSaving(false);
            setMessage({ type: "error", text: "Slug oluşturulamadı. Lütfen elle girin." });
            return;
        }
        ...
        try {
            ...
            const result = (await response.json()) as ProjectMutationResponse;
            ...
            setMessage({
                type: "success",
                text: form.id ? "Proje güncellendi." : "Proje oluşturuldu.",
            });
        } catch (error) {
            setMessage({
                type: "error",
    ...
    async function deleteProject(project: ProjectRecord) {
        ...
        try {
            ...
            if (!response.ok) {
                setMessage({ type: "error", text: result.error ?? "Proje silinemedi." });
                return;
            }

            setProjects((current) => current.filter((item) => item.id !== project.id));
            setMessage({ type: "success", text: "Proje silindi." });
            ...
            setMessage({
                type: "error",
                text: error instanceof Error ? error.message : "Proje silinemedi.",
            });
        } finally {
            setDeletingId(null);
        }
    ...
    return (
        <div className="space-y-6">
            ...
            <section className="grid gap-4 md:grid-cols-3">
                <StatCard label="Toplam" value={projects.length.toString()} />
                <StatCard label="Yayında" value={publishedCount.toString()} />
                <StatCard label="Favori" value={favoriteCount.toString()} />
            </section>
            ...
            <div className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-white/[0.04]">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <h2 className="text-lg font-semibold">Proje Listesi</h2>
                ...
                <div className="mt-5 overflow-hidden rounded-2xl border border-zinc-200 dark:border-white/10">
                    {projects.length === 0 ? (
                        <div className="flex min-h-64 flex-col items-center justify-center gap-3 p-8 text-center">
                            ...
                            <div>
                                <h3 className="font-semibold">Henüz proje yok</h3>
                                <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
                                    İlk projeyi ekleyerek public site verisini hazırlayabilirsiniz.
                                </p>
                            </div>
                        </div>
                    ) : (
                    ...
                    <div className="flex flex-col gap-4 border-b border-zinc-200 pb-5 dark:border-white/10 sm:flex-row sm:items-start sm:justify-between">
                        <div>
                            <p className="text-sm text-zinc-500 dark:text-zinc-400">
                                {form.id ? "Proje düzenleme" : "Yeni proje"}
                            </p>
                            <h2 className="mt-1 text-xl font-semibold">
                                {form.id ? "Proje bilgilerini güncelle" : "Proje oluştur"}
                            </h2>
                        </div>
                        ...
                        <aside className="space-y-4 rounded-2xl border border-zinc-200 p-4 dark:border-white/10">
                            <div className="space-y-2">
                                ...
                                <Input
                                    ...
                                    placeholder="ankara-celik-konstruksiyon"
                                />
                            ...
                            <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-1">
                                ...
                                <div className="space-y-2">
                                    <label htmlFor="weight_tons" className="text-sm font-medium">Ağırlık Ton</label>
                                    <Input
                                        id="weight_tons"
                                        type="number"
                                        step="0.01"
                            ...
                            <div className="space-y-3 rounded-2xl bg-zinc-50 p-4 dark:bg-white/5">
                                ...
                                <SwitchRow
                                    label="Favori"
                                    description="Öne çıkan projeler alanında kullanılabilir."
                                    checked={form.is_favorite}
                                    onChange={(value) => setForm((current) => ({ ...current, is_favorite: value }))}
                                />
                            </div>
                        </aside>
                    </div>
...
function ProjectRow({
    ...
    const title = getProjectTitle(project);
    const photo = project.main_photo || project.project_photos[0]?.url;
        ...
        >
            ...
            <div className="min-w-0">
                ...
                <div className="mt-3 flex flex-wrap gap-2 text-xs text-zinc-500 dark:text-zinc-400">
                    ...
                    <span>Ağırlık: {project.weight_tons ?? "-"} ton</span>
                    <span>Sıra: {project.sort_order}</span>
                    <span>Fotoğraf: {project.project_photos.length}</span>
                </div>
            </div>
...
function getProjectTitle(project: ProjectRecord) {
    return (
        project.project_translations.find((item) => item.locale === "tr")?.title ||
        project.project_translations.find((item) => item.locale === "en")?.title ||
        project.slug
    );
}
...
function getUploadFolder(form: ProjectFormState) {
    const folder = form.id ?? slugify(form.slug || form.translations.tr.title || form.translations.en.title);

    return folder ? `stallcons/projects/${folder}` : "stallcons/projects";
}
...
function toUploadFileList(urls: string[]): ImageUploadFile[] {
    return urls
        ...
        .map((url, index) => ({
            uid: url,
            name: getFileNameFromUrl(url) || `project-photo-${index + 1}`,
            status: "done",
            url,
            response: { url },
        }));
